...
export default function AccountDetailPage() {
  ...
  const loadData = async (vaultId: string) => {
    ...
    // 2. TOUTES les transactions de ce coffre (uniquement VALIDATED pour le solde réel)
    ...
    <div className="h-[calc(100vh-60px)] bg-gray-100 p-2 text-xs font-sans overflow-hidden flex flex-col">
      ...
      <div className="bg-white border-b border-gray-300 p-2 flex justify-between items-center shadow-sm h-12 shrink-0 no-print">
        <div className="flex items-center gap-3">
            <button onClick={() => router.back()} className="p-1 hover:bg-gray-100 rounded text-gray-600 transition"><ArrowLeft size={16}/></button>
            <div>
                ...
                <p className="text-[10px] text-gray-500 font-mono">ID: {vault.id.split('-')[0]}</p>
            </div>
        </div>
        ...
        <div className="flex items-center gap-4">
             ...
             <div className="text-right border-r pr-4 border-gray-200">
                <span className="text-[9px] font-bold text-gray-400 uppercase block">Solde Actuel</span>
                <span className={`text-lg font-bold font-mono ${realBalance < 0 ? 'text-red-600' : 'text-blue-700'}`}>
            ...
            <button onClick={() => window.print()} className="bg-gray-800 text-white px-3 py-1.5 rounded text-[10px] font-bold flex items-center gap-2 hover:bg-black transition active:scale-95">
                <Printer size={12}/> RELEVÉ
            </button>
            <button onClick={() => loadData(id)}><RefreshCw size={14} className="text-blue-600 hover:rotate-180 transition"/></button>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white border border-gray-400 rounded shadow-sm flex flex-col mt-2 overflow-hidden print:border-none print:shadow-none">
        ...
        <div className="hidden print:block text-center mb-6 border-b pb-4 pt-4">
            ...
            <p className="mt-2 font-bold border-2 border-black inline-block px-4 py-1">Solde au {new Date().toLocaleDateString()} : {realBalance.toLocaleString()} Ar</p>
        </div>
            ...
            <table className="w-full text-left text-[11px] border-collapse">
                <thead className="bg-gray-200 text-gray-600 font-bold uppercase sticky top-0 z-10 shadow-sm text-[9px]">
                    <tr>
                        ...
                        <th className="p-2 border-r border-gray-300 w-32 text-right bg-green-50">Crédit (+)</th>
                        <th className="p-2 border-r border-gray-300 w-32 text-right bg-red-50">Débit (-)</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 font-mono">
                        ...
                        return (
                            <tr key={t.id} className={`${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-yellow-50 transition-colors print:bg-white`}>
                                <td className="p-2 border-r border-gray-100 text-gray-500">
                                    ...
                                    <span className="text-[9px] block opacity-50">{new Date(t.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                </td>
                                ...
                                <td className="p-2 border-r border-gray-100 font-medium text-gray-700 font-sans">
                                    <span className="block font-bold text-[9px] text-gray-400 uppercase">{t.category}</span>
                                    {t.description}
                                </td>
                    ...
                    {filtered.length === 0 && (
                        <tr><td colSpan={5} className="p-8 text-center text-gray-400 italic">Aucune opération sur cette période.</td></tr>
                    )}
                </tbody>
            </table>
        </div>
        ...
        <div className="bg-gray-100 font-bold border-t border-gray-300 flex text-[10px] uppercase shrink-0">
            <div className="p-2 flex-1 text-right text-gray-600 border-r border-gray-300">Total Mouvements (Période) :</div>
            <div className="p-2 w-32 text-right text-green-800 border-r border-gray-300 bg-green-50">
                +{filtered.reduce((acc, t) => {
                     if (t.type === 'RECETTE' || (t.type === 'TRANSFERT' && t.amount > 0)) return acc + t.amount;
                     return acc;
                }, 0).toLocaleString()}
            </div>
            <div className="p-2 w-32 text-right text-red-800 border-r border-gray-300 bg-red-50">
                -{filtered.reduce((acc, t) => {
      ...
      <style jsx global>{`
        @media print {
            ...
            div { height: auto !important; overflow: visible !important; }
        }
      `}</style>
    </div>
  );
